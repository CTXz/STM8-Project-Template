...
# Dead Code Eliminaton related definitions
# Symbols to be excluded from dead code elimination
# Unfortunatelly the DCE tool is not very smart and will mistakely exclude
...
# The default values provided assume a STM8S103F3 target device and the stlinkv2 as programmer
...
toolchain_check:
	...
	@echo "----------------------------------------"
	@$(CC) -v >/dev/null 2>&1 || \
		(echo "ERROR: Toolchain not found. Please install the SDCC compiler from https://sourceforge.net/projects/sdcc/files/sdcc-linux-amd64/4.0.0/sdcc-4.0.0-amd64-unknown-linux2.5.tar.bz2/download" && exit 1)
	@$(FLASH_TOOL) -V >/dev/null 2>&1 || \
		(echo "ERROR: Flash tool not found. Please install stm8flash from
